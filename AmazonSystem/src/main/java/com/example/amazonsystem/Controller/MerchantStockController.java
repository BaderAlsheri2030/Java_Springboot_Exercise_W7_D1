...
public class MerchantStockController {
    ...
    public ResponseEntity addMerchantStock(@PathVariable Integer pID,@PathVariable Integer mID, @Valid @RequestBody MerchantStock merchantStock, Errors errors){
        if (errors.hasErrors()){
            String message = errors.getFieldError().getDefaultMessage();
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(message);
        }
        merchantStockService.addMerchantStock(mID,pID,merchantStock);
